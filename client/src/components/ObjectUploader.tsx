...
export function ObjectUploader({
  ...
  buttonClassName,
  children,
}: ObjectUploaderProps & { onError?: (file: UppyFile<Record<string, unknown>, Record<string, unknown>> | undefined, error: Error) => void }) {
  const [showModal, setShowModal] = useState(false);
  const { toast } = useToast();
  
  // Store callbacks in refs so Uppy always has access to the latest versions
  ...
  }, [onGetUploadParameters, onComplete, onError]);
  ...
  const [uppy] = useState(() =>
    new Uppy({
      ...
      .on("error", (error) => {
        ...
        toast({
          title: "Upload failed",
          description: "The file could not be uploaded. Please try again or use a smaller file.",
          variant: "destructive",
        });
      })
      .on("upload-error", (file, error) => {
        ...
        toast({
          title: "Upload failed",
          description: `Failed to upload ${file?.name || 'file'}. Try a smaller file or check your connection.`,
          variant: "destructive",
        });
      })
  );
  
  // Update Uppy restrictions when maxFileSize or maxNumberOfFiles change
  useEffect(() => {
    uppy.setOptions({
      restrictions: {
        maxNumberOfFiles,
